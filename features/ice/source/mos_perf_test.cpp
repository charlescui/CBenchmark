...
void* mos_getUserStatus(void *p)
{
	...
	::AMOServer::SICEMosStatusSeq::iterator mos_ite;
	...
	try{
		...
		for(mos_ite=mosStatusList.begin();mos_ite != mosStatusList.end(); ++mos_ite){
			cout << "userId = " << mos_ite->userId  << " status = " << mos_ite->status << endl;
		}
	}catch(const Ice::Exception& ex){
		cerr << ex << endl;
		status = 1;
