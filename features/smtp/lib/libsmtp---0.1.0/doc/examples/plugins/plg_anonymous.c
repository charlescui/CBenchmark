 ...
 *                                                                      *
 * compile with:                                                        *
 * gcc -shared -lsmtp-- plg_anonymous.c -o plg_anonymous.so             *
 *                                                                      *
 ************************************************************************/
...
#include <libsmtp--/alloc.h>
#include <libsmtp--/inetsocket.h>

/**
 * The name of the plugin. NAME should not be NULL.
 */
const char *NAME = "SASL plugin ANONYMOUS - Ver. 1.0";
...
int plg_authenticate (smtp_srv_t * srv)
{
    ...
    if (smtp_write (srv, "AUTH ANONYMOUS ", 15) == -1)
    {
        free_ (enc, NULL);
