 ...
 */
const char *NAME = "SASL plugin CRAM-MD5 - Ver. 1.0";
...
char * plg_response (smtp_srv_t * srv, char * msg, int *siz)
{
    ...
    if (strlen (msg) <= 4)
    {
        err_set_error ("Invalid CRAM-MD5 challenge received.", NULL);
    ...
    alg_hmac ((unsigned char*)buf, size, 
              ...
              (unsigned char*)dig, H_MD5);
    free_ (buf, NULL);

    str_hexify (hex_dig, dig, 16);

    /* Response: BASE64("USERNAME HEX_DIGEST") */
    buf = (char*)malloc_ (usr_len + 34);
    sprintf (buf, "%s %.32s", srv->username, hex_dig);

    /* BASE64 encoding */
    enc_buf = (char*)malloc_ (2 * (usr_len + 33) + 4);
